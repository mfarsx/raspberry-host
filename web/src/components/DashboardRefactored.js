    ...
    <div>
      ...
      <div className="card">
        ...
        <div className="grid grid-4">
          <div className="text-center">
            <h3>API Health</h3>
            <div className={`status-indicator ${status?.ok ? 'status-online' : 'status-offline'}`} 
                 style={{ width: '24px', height: '24px', margin: '0 auto 10px' }}></div>
          ...
          <div className="text-center">
            <h3>WebSocket</h3>
            <div className={`status-indicator ${socketConnected ? 'status-online' : 'status-offline'}`} 
                 style={{ width: '24px', height: '24px', margin: '0 auto 10px' }}></div>
          ...
          <div className="text-center">
            <h3>Database</h3>
            <div className={`status-indicator ${status?.database?.connected ? 'status-online' : 'status-offline'}`} 
                 style={{ width: '24px', height: '24px', margin: '0 auto 10px' }}></div>
          ...
          <div className="text-center">
            <h3>Redis</h3>
            <div className={`status-indicator ${status?.redis?.connected ? 'status-online' : 'status-offline'}`} 
                 style={{ width: '24px', height: '24px', margin: '0 auto 10px' }}></div>
              ...
              <div key={project.id} className="border rounded-lg p-4">
                ...
                <div className="text-xs text-gray-500">
                  ...
                  {project.lastDeployed && (
                    <div>Deployed: {new Date(project.lastDeployed).toLocaleDateString()}</div>
                  )}
                </div>
              </div>
            ))}
              ...
              {status.memory?.used && status.memory?.total && (
                <div className="mt-2">
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    {Math.round((status.memory.used / status.memory.total) * 100)}% used
                  </p>
                </div>
              )}
            </div>
